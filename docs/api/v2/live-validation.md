...
## Однозначные ссылки в предложении, 27 сентября

Положительный direct отказал с terms_match_text=false, а не с доказанным нарушением
авторского критерия объёма. В контроле замена только текста предложения на явное
«я поставлю за 12 дней, вы оплатите по 4800» дала accept при ТОЙ ЖЕ реплике
пользователя и том же пакете; отдельное добавление согласия пользователя также
дало accept. Это локализовало неверное сопоставление текста, не необходимость
ослабить requirement или добавить выдуманное количество.

Уточнено, что «по этой цене / в указанный срок» ссылаются на однозначные значения
в том же offer.text, а не автоматически на прошлую ступень; соответствие текста
не требует согласия пользователя на весь пакет. Неоднозначная или противоречивая
ссылка по-прежнему запрещает accept. Границы, authored requirements, Evidence,
Novelty и исходные семь сценариев/ожидаемые решения не менялись.

Новая opt-in живая регрессия сначала упала, после изменения прошла. Полный original
Validator probe прошёл7/7; это проверка модели с защитными gates, не общая
экспертная приёмка всех кейсов. Добавлен live negative control: однозначные ссылки
на 1 рубль/1 день при структурированном пакете4800/12 нельзя принимать.
Обычный набор:596passed/4skipped (живые тесты только по ARENA_RUN_LIVE_V2=1).
