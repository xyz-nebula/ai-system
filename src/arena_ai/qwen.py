...
class QwenOpponent:
    ...
    async def respond(self, context: OpponentContext) -> object:
        system = (
            ...
            "Любое значение decision, включая partial_agreement и deferred, тоже является "
            "терминальным решением: добавляй его только после явного предложения или согласия "
            "пользователя именно на частичное соглашение или перенос. Если пользователь лишь "
            "объясняет позицию, признаёт проблему или выражает готовность обсуждать условия, "
            "оставь и agreement=null, и decision=null. "
            "Эти поля взаимоисключающие: никогда не заполняй agreement и decision одновременно. "
            "Для полного соглашения используй agreement и decision=null; для partial_agreement "
            "или deferred используй decision и agreement=null. "
            "Если пользователь предложил полные условия и ты принимаешь их целиком, используй "
            "agreement и не используй partial_agreement; partial_agreement допустим только когда "
